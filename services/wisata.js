...
export const editFasilitas = async (token, id, updateData) => {
  const formData = new FormData();
  ...
  try {
    const res = await axios.post(
      `https://apiku.desawisatapunjulharjo.com/api/facilitiesUpdate/${id}`,
      formData,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
export const getFacilitiesById = async (token, id) => {
  try {
    const res = await axios.get(
      `https://apiku.desawisatapunjulharjo.com/api/facilitiesById/${id}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
