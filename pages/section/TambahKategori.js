...
import { addKategori } from "@/services/wisata";
import PopUp from "@/components/Popup";

const TambahKategori = ({ isAdmin }) => {
  ...
  const handleAddKategori = async (e) => {
    ...
    const res = await addKategori(token, kategori);
    setLoading(false);
    if (res) {
      ...
      setShowPopup(true);
      setTimeout(() => {
        router.push("/manage-wisata"); // Change route to "/manage-produk"
      }, 1500);
    } else {
      ...
      <div className="sm:p-2">
        <div className="flex mb-4 items-center">
          ...
          <h2 className="text-black ml-2 text-2xl font-semibold">
            {isAdmin ? "Tambah Kategori" : "Tambah Kategori"}
          </h2>
        </div>
