...
import { editFasilitas, getFacilitiesById } from "@/services/wisata";
...
const UpdateFasilitas = ({ isAdmin }) => {
  ...
  const handleEditVideo = async (e) => {
    ...
    const res = await editFasilitas(token, id, updateData);
    if (res) {
      console.log("Video updated:", res);
      setPopupText("Fasilitas Berhasil Diupdate");
      setPopupType("success");
      setShowPopup(true);
      setTimeout(() => {
        router.push(`/manage-fasilitas/${wisataId}`);
      }, 1500);
    } else {
      console.log("Failed to update video");
      setPopupText("Fasilitas Gagal Diupdate");
      setPopupType("error");
      setShowPopup(true);
    }
  };
  ...
  useEffect(() => {
    if (id && token) {
      const fetchVideo = async () => {
        try {
          const data = await getFacilitiesById(token, id);
          setTitle(data.title);
      ...
      <div className="mt-2 bg-white p-4 shadow-sm rounded-md max-w-7xl mx-auto">
        <div className="flex mb-4 items-center">
          <div className="text-primary hover:opacity-90">
            <Link href={`/manage-fasilitas/${wisataId}`}>
              <Icon
          ...
          <h2 className="text-black ml-2 text-2xl font-semibold">
            {isAdmin ? "Update Fasilitas" : "Update Fasilitas"}
          </h2>
        </div>
